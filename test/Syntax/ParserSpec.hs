...
derivedSymbolSpec = do
    describe "derivedSymbol" $ do
        it "parses read" $ do
            parseEvalState (S.fromList ["tape"]) derivedSymbol "" "read tape" `shouldParse` (Read "tape")

        it "parses variable names" $ do
            parseEvalState (S.fromList ["x"]) derivedSymbol "" "x" `shouldParse` Var "x"
...
bexpSpec = describe "bexp" $ do
    ...
    context "parsing EQ operator" $ do
        it "parses EQ" $ do
            parseEvalState (S.fromList ["tape", "x"]) bexp "" "x == read tape" `shouldParse` (Eq (Var "x") (Read "tape"))

        it "fails to parse chains" $ do
            parseEvalState (S.fromList ["x", "y", "z"]) bexp "" "'x' == y == z" `shouldParse` (Eq (Literal 'x') (Var "y"))

    context "parsing LE operator" $ do
        it "parses LE" $ do
            parseEvalState (S.fromList ["x", "tape"]) bexp "" "x <= read tape" `shouldParse` (Le (Var "x") (Read "tape"))

        it "fails to parse chains" $ do
            parseEvalState (S.fromList ["x", "y", "z"]) bexp "" "x <= y <= z" `shouldParse` (Le (Var "x") (Var "y"))

    context "parsing NE operator" $ do
        it "parses NE" $ do
            parseEvalState (S.fromList ["x", "tape"]) bexp "" "x != read tape" `shouldParse` (Ne (Var "x") (Read "tape"))

        it "fails to parse chains" $ do
            parseEvalState (S.fromList ["x", "y", "z"]) bexp "" "x != y != z" `shouldParse` (Ne (Var "x") (Var "y"))
...
ifStmSpec = describe "ifStm" $ do
    context "parsing a single IF" $ do
        it "parses IF" $ do
            parseEvalState (S.fromList ["tape"]) program "" "if True { right tape }" `shouldParseStm` (If TRUE (MoveRight "tape") [] Nothing)

        it "fails to parse if a boolean expression is missing" $ do
            parseEvalState (S.fromList ["tape"]) program "" `shouldFailOn` "if { right tape }"

        it "fails to parse if the first brace is missing" $ do
            parseEvalState (S.fromList ["tape"]) program "" `shouldFailOn` "if True right tape }"
    ...
    context "parsing an IF-ELSEIF" $ do
        it "parses with a single ELSE-IF clause" $ do
            ...
            parseEvalState (S.fromList ["tape"]) program "" str `shouldParseStm` expected
        ...
        it "fails to parse if ELSE-IF is before IF" $ do
            parseEvalState (S.fromList ["tape"]) program "" `shouldFailOn` "else if True { right tape } if True right tape }"

        it "fails to parse if the first brace is missing" $ do
            parseEvalState (S.fromList ["tape"]) program "" `shouldFailOn` "if True { right tape } else if True right tape }"

        it "fails to parse if the second brace is missing" $ do
            parseEvalState (S.fromList ["tape"]) program "" `shouldFailOn` "if True { right tape } else if True { right tape"

        it "fails to parse if both braces are missing" $ do
            parseEvalState (S.fromList ["tape"]) program "" `shouldFailOn` "if True { right tape } else if True right rape"
    ...
    context "parsing an ELSE clause" $ do
        ...
        it "fails to parse if the ELSE is before IF" $ do
            parseEvalState (S.fromList ["tape"]) program "" `shouldFailOn` "else { accept } if { left tape }"

        it "fails to parse if the first brace is missing" $ do
            parseEvalState (S.fromList ["tape"]) program "" `shouldFailOn` "if True { right tape } else right tape }"
...
programSpec = describe "program" $ do
    context "parsing Turing Machine operators" $ do
        it "parses LEFT command" $ do
            parseEvalState (S.fromList ["tape"]) program "" "left tape" `shouldParseStm` (MoveLeft "tape")

        it "parses RIGHT command" $ do
            parseEvalState (S.fromList ["tape"]) program "" "right tape" `shouldParseStm` (MoveRight "tape")

        it "parses WRITE command" $ do
            parseEvalState (S.fromList ["tape"]) program "" "write tape 'x'" `shouldParseStm` (Write "tape" (Literal 'x'))

        it "parses a WRITESTR command" $ do
            parseEvalState (S.fromList ["tape"]) program "" "write tape \"abcd\"" `shouldParseStm` (WriteStr "tape" "abcd")
    ...
    context "parsing variable declarations" $ do
        it "parses variable declarations" $ do
            parseEvalState (S.fromList ["tape"]) program "" "let x = read tape" `shouldParseStm` VarDecl "x" (Read "tape")

        it "fails if '=' is missing" $ do
            parseEvalState (S.fromList ["tape"]) program "" `shouldFailOn` "let x read tape"
    ...
    context "parsing WHILE statements" $ do
        it "parses WHILE" $ do
            parseEvalState (S.fromList ["tape"]) program "" "while True { right tape }" `shouldParseStm` (While TRUE (MoveRight "tape"))

        it "fails to parse if a boolean expression is missing" $ do
            parseEvalState (S.fromList ["tape"]) program "" `shouldFailOn` "while { right tape }"

        it "fails to parse if the first brace is missing" $ do
            parseEvalState (S.fromList ["tape"]) program "" `shouldFailOn` "while True right tape }"
    ...
    context "parsing function declarations" $ do
        it "parses function delcarations" $ do
            ...
            parseEvalState (S.fromList ["tape"]) program "" "func fName { right tape }" `shouldParseStm` expected
        ...
        it "parses function declarations with arguments" $ do
            ...
            parseEvalState (S.fromList ["tape"]) program "" "func fName a:Sym bb:Tape { right tape }" `shouldParseStm` expected

        it "parses function declarations where the name contains a keyword" $ do
            let expected = FuncDecl "leftUntil" [] (MoveRight "tape")
            parseEvalState (S.fromList ["tape"]) program "" "func leftUntil { right tape }" `shouldParseStm` expected

        it "fails to parse if a function name is missing" $ do
            parseEvalState (S.fromList ["tape"]) program "" `shouldFailOn` "func { right tape }"

        it "fails to parse if the first brace is missing" $ do
            parseEvalState (S.fromList ["tape"]) program "" `shouldFailOn` "func fName right tape }"
    ...
    context "parsing function calls" $ do
        ...
        it "parses function calls with arguments" $ do
            ...
            parseEvalState (S.fromList ["tape", "x"]) program "" "fName (read tape) x '#'" `shouldParseStm` expected
        ...
        it "parses function calls followed by another statement" $ do
            ...
            parseEvalState (S.fromList ["tape"]) program "" "fName (read tape) \n left tape" `shouldParseStm` expected
    ...
    context "parsing composition" $ do
        ...
        it "fails if parsing the first statements fails to parse" $ do
            parseEvalState (S.fromList ["tape"]) program "" `shouldFailOn` "left tape \n if"

        it "fails if parsing the second statements fails to parse" $ do
            parseEvalState (S.fromList ["tape"]) program "" `shouldFailOn `"if \n left tape"
    ...
    context "parsing printing" $ do
        ...
        it "parses printing the symbol read from the tape" $ do
            parseEvalState (S.fromList ["tape"]) program "" "print tape" `shouldParseStm` (PrintRead "tape")
    ...
    context "removing whitespace and comments" $ do
        context "before statements" $ do
            ...
            it "ignores newlines" $ do
                parseEvalState (S.fromList ["tape"]) program "" "\n\nleft tape" `shouldParseStm` (MoveLeft "tape")

            it "ignores whole-line comments" $ do
                parseEvalState (S.fromList ["tape"]) program "" "//Comment\n left tape" `shouldParseStm` (MoveLeft "tape")
        ...
        context "interspersed with statements" $ do
            ...
            it "ignores in-line comments" $ do
                parseEvalState (S.fromList ["tape"]) program "" "if /* Comment */ True { left tape }" `shouldParseStm` (If TRUE (MoveLeft "tape") [] Nothing)
        ...
        context "whitespace nested in statements" $ do
            it "ignores newlines after an opening brace" $ do
                ...
                parseEvalState (S.fromList ["tape"]) program "" "while True {\n\n left tape }" `shouldParseStm` expected

            it "ignores newlines before a closing brace" $ do
                let expected = While TRUE (MoveLeft "tape")
                parseEvalState (S.fromList ["tape"]) program "" "while True { left tape \n\n }" `shouldParseStm` expected
