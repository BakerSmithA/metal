...
evalDerivedSymbol sym config = fmap (fmap fst) (evalApp (derivedSymbolVal sym (return config)))

-- Runs `bexpVal` with `b` in the given config and environment.
evalBexp :: Bexp -> Config -> StateResult Bool
evalBexp b config = fmap (fmap fst) (evalApp (bexpVal b (return config)))

-- Runs `s` in config with the empty environment.
evalSemantics :: Stm -> Config -> StateResultConfig
evalSemantics s config = fmap (fmap fst) (evalApp (evalStm s (return config)))
