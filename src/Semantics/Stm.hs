...
evalIf :: (Monad m) => Bexp -> Stm -> [(Bexp, Stm)] -> Maybe Stm -> Config -> App m Config
...
evalWhile :: (Monad m) => Bexp -> Stm -> Config -> App m Config
...
-- the body.
evalFuncBody :: (Monad m) => FuncName -> FuncDeclArgs -> FuncCallArgs -> Stm -> Config -> App m Config
...
evalCall :: (Monad m) => FuncName -> FuncCallArgs -> Config -> App m Config
